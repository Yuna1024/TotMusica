...
export const registerUser = async (user, dispatch, history) => {
  ...
  try {
    const response = await axios.post("http://localhost:8000/v1/auth/register", user);
...
export const ReceiveCode = async (email) => {
  try {
    ...
    const response = await axios.post("http://localhost:8000/email/send-verification-code", {
      email,
    });
...
export const ConfirmCode = async (email, verificationCode) => {
  try {
    const response = await axios.post("http://localhost:8000/email/verify-code", {
      email,
...
export const checkEmailExistence = async (email) => {
  try {
    const response = await axios.post("http://localhost:8000/email/check-email", {
      email,
    });
...
export const changePasswordService = async (newPassword, history) => {
  try {
    const response = await axios.post("http://localhost:8000/v1/auth/changepassword", {
      newPassword,
    });
    if (response.status === 200) {
      history.push("/LoginPage");
...
export const updateProfile = (userData, history) => async (dispatch) => {
  ...
  try {
    const response = await axios.put(`http://localhost:8000/v1//user/${userData.id}`, userData);
